// MVP 프로젝트 데이터 - 리뉴얼 프로젝트 컬렉션
export interface Project {
  id: string;
  title: string;
  description: string;
  url: string;
  category: string;
  date: string;
  tags: string[];
  featured?: boolean;
}

export const projects: Project[] = [
  // 🔥 2025년 리뉴얼 프로젝트 컬렉션 (10월)
  {
    id: "mvp-04-baical-renewal",
    title: "바이칼시스템즈 홈페이지",
    description: "시스템 솔루션 기업의 브랜드와 기술력을 소개하는 현대적 홈페이지 리뉴얼",
    url: "https://mvp-project-04-qxxrfyiqy-dongyeol-jungs-projects.vercel.app/",
    category: "웹사이트",
    date: "2025-10-08",
    tags: ["기업홈페이지", "시스템솔루션", "리뉴얼", "브랜딩"],
    featured: true
  },
  {
    id: "mvp-09-mansong-renewal",
    title: "만송시스템 홈페이지",
    description: "기업 브랜딩과 서비스 소개를 위한 현대적 홈페이지 리뉴얼",
    url: "https://mvp-project-09-mkg5tmd00-dongyeol-jungs-projects.vercel.app/",
    category: "웹사이트",
    date: "2025-10-03",
    tags: ["기업홈페이지", "브랜딩", "리뉴얼", "서비스소개"],
    featured: true
  },
  {
    id: "mxten-15-portfolio",
    title: "바이브 코딩 MVP 소개자료 웹앱",
    description: "27개 리뉴얼 프로젝트를 소개하는 포트폴리오 웹애플리케이션",
    url: "https://mxten-project-15-48zl7ce5t-dongyeol-jungs-projects.vercel.app/",
    category: "포트폴리오",
    date: "2025-10-12",
    tags: ["포트폴리오", "MVP", "프레젠테이션", "React"],
    featured: true
  },
  {
    id: "dbinfo-renewal",
    title: "디비인포 리뉴얼",
    description: "데이터베이스 정보 서비스 플랫폼 완전 리뉴얼",
    url: "https://dbinfo-homepage-cy6dezpsv-dongyeol-jungs-projects.vercel.app/",
    category: "플랫폼",
    date: "2025-10-08",
    tags: ["데이터베이스", "정보서비스", "리뉴얼", "플랫폼"]
  },
  {
    id: "dbinfo-mvp",
    title: "디비인포 MVP 버전",
    description: "디비인포 서비스의 MVP 버전 - 핵심 기능 중심",
    url: "https://dbinfo-homepage.vercel.app/",
    category: "플랫폼",
    date: "2025-10-08",
    tags: ["MVP", "데이터베이스", "핵심기능"]
  },
  {
    id: "dbinfo-admin",
    title: "디비인포 관리자 모드",
    description: "디비인포 관리자 전용 관리 시스템 (비밀번호: admin1234!)",
    url: "https://dbinfo-homepage.vercel.app/admin/login",
    category: "관리시스템",
    date: "2025-10-08",
    tags: ["관리자", "백오피스", "관리시스템"]
  },
  {
    id: "mxten-06-work-order",
    title: "모바일/태블릿 기반 디지털 작업지시서",
    description: "현장 작업자를 위한 모바일 최적화 디지털 작업 지시 시스템",
    url: "https://mxten-project-06-6xjxhg48i-dongyeol-jungs-projects.vercel.app/",
    category: "업무도구",
    date: "2025-10-04",
    tags: ["모바일", "태블릿", "작업지시서", "현장관리"]
  },
  {
    id: "lawmaker-landing-renewal",
    title: "국회의원 랜딩페이지 MVP",
    description: "시민과 함께하는 더 나은 미래를 위한 국회의원 공식 랜딩페이지",
    url: "https://lawmaker-landing-akik9rar6-dongyeol-jungs-projects.vercel.app/",
    category: "공공서비스",
    date: "2025-10-03",
    tags: ["정치", "랜딩페이지", "시민참여", "공공서비스"],
    featured: true
  },
  {
    id: "mvp-26-resort-renewal",
    title: "바이칼 리조트 홈페이지 및 예약시스템",
    description: "리조트 소개와 실시간 예약이 가능한 통합 시스템 리뉴얼",
    url: "https://mvp-project-26-7iwsc36ux-dongyeol-jungs-projects.vercel.app/",
    category: "숙박/예약",
    date: "2025-10-03",
    tags: ["리조트", "예약시스템", "숙박", "관광", "리뉴얼"]
  },
  {
    id: "mvp-03-kopron-renewal",
    title: "한국코프론 홈페이지",
    description: "기업 정보와 제품 소개를 위한 반응형 웹사이트 리뉴얼",
    url: "https://mvp-project-03-jhbjp3e6e-dongyeol-jungs-projects.vercel.app/",
    category: "웹사이트",
    date: "2025-10-03",
    tags: ["기업정보", "제품소개", "반응형", "리뉴얼"]
  },
  {
    id: "daycare-renewal",
    title: "아이뜨락 어린이집 홈페이지",
    description: "어린이집 소개와 교육 프로그램을 안내하는 웹사이트 리뉴얼",
    url: "https://new-project-02-jj8qpb4j0-dongyeol-jungs-projects.vercel.app/",
    category: "교육",
    date: "2025-10-04",
    tags: ["어린이집", "교육프로그램", "학부모", "리뉴얼"]
  },
  {
    id: "welfare-center-renewal",
    title: "바이칼 재가복지센터 홈페이지",
    description: "재가복지센터의 서비스와 프로그램을 소개하는 홈페이지 리뉴얼",
    url: "https://mvp-project-18-n9qp4huj7-dongyeol-jungs-projects.vercel.app/",
    category: "복지",
    date: "2025-10-05",
    tags: ["복지센터", "재가복지", "사회서비스", "리뉴얼"]
  },
  {
    id: "admin-office-renewal",
    title: "박신환 행정사 홈페이지",
    description: "행정사 사무소의 전문적인 서비스 소개 홈페이지 리뉴얼",
    url: "https://new-project-04-mwevrreb4-dongyeol-jungs-projects.vercel.app/",
    category: "웹사이트",
    date: "2025-10-10",
    tags: ["행정사", "법무서비스", "전문직", "리뉴얼"]
  },
  {
    id: "dental-clinic-renewal",
    title: "박영진치과 홈페이지",
    description: "치과 의료진과 진료 서비스를 소개하는 전문 의료 홈페이지 리뉴얼",
    url: "https://park-youngjin-dental-nu9npms7j-dongyeol-jungs-projects.vercel.app/",
    category: "의료",
    date: "2025-10-10",
    tags: ["치과", "의료", "진료서비스", "리뉴얼"],
    featured: true
  },
  {
    id: "ent-clinic-renewal",
    title: "선우 이비인후과 홈페이지",
    description: "전문적인 이비인후과 진료 서비스와 의료진을 소개하는 홈페이지",
    url: "https://new-project-30-6me2stquj-dongyeol-jungs-projects.vercel.app/",
    category: "의료",
    date: "2025-10-10",
    tags: ["이비인후과", "의료", "진료서비스", "전문의"]
  },
  {
    id: "orthopedic-clinic-renewal",
    title: "성혜 정형외과 의원 프리미엄 웹사이트",
    description: "정형외과 전문 진료와 AI 증상체크, 실시간 예약이 가능한 프리미엄 의료 웹사이트",
    url: "https://new-project-40-im6ku6iue-dongyeol-jungs-projects.vercel.app/",
    category: "의료",
    date: "2025-10-10",
    tags: ["정형외과", "AI증상체크", "실시간예약", "프리미엄"],
    featured: true
  },
  {
    id: "rpa-platform-renewal",
    title: "시군구 RPA 통합 플랫폼 앱",
    description: "지방자치단체의 업무 자동화를 위한 RPA 통합 관리 플랫폼 리뉴얼",
    url: "https://mvp-project-14-9bmios8kd-dongyeol-jungs-projects.vercel.app/",
    category: "공공서비스",
    date: "2025-10-10",
    tags: ["RPA", "자동화", "지방자치", "공공서비스"],
    featured: true
  },
  {
    id: "caring-plus-renewal",
    title: "재가 복지 센터 통합 관리 시스템",
    description: "재가복지센터를 위한 통합 관리 및 서비스 운영 시스템",
    url: "https://caring-plus-1usacqdhl-dongyeol-jungs-projects.vercel.app/login",
    category: "복지",
    date: "2025-10-10",
    tags: ["복지관리", "통합시스템", "재가복지", "관리플랫폼"]
  },
  {
    id: "public-facility-booking",
    title: "공공시설 예약 통합 플랫폼",
    description: "지역 공공시설을 통합적으로 예약할 수 있는 플랫폼",
    url: "https://mvp-project-28-h21qsrg0t-dongyeol-jungs-projects.vercel.app/",
    category: "공공서비스",
    date: "2025-10-11",
    tags: ["공공시설", "예약시스템", "통합플랫폼", "지역서비스"]
  },
  {
    id: "medicine-reminder",
    title: "약 복용관리 플랫폼",
    description: "복용 리마인더와 가족 알림 기능이 있는 약물 관리 플랫폼",
    url: "https://mvp-project-10-g3ca25sv4-dongyeol-jungs-projects.vercel.app/",
    category: "헬스케어",
    date: "2025-10-11",
    tags: ["약복용", "리마인더", "가족알림", "건강관리"]
  },
  {
    id: "pachinko-game",
    title: "슬롯머신형 일본 파칭코 웹 MVP",
    description: "재미있는 슬롯머신 스타일의 파칭코 게임 웹 애플리케이션",
    url: "https://mxten-project-10-4mmti96wq-dongyeol-jungs-projects.vercel.app/",
    category: "게임",
    date: "2025-10-11",
    tags: ["파칭코", "슬롯머신", "게임", "엔터테인먼트"]
  },
  {
    id: "complaint-system",
    title: "주민 제보·민원 실시간 처리 웹앱",
    description: "주민들의 제보와 민원을 실시간으로 처리하는 공공 서비스",
    url: "https://mvp-project-30-czlodp7s2-dongyeol-jungs-projects.vercel.app/",
    category: "공공서비스",
    date: "2025-10-11",
    tags: ["민원처리", "제보시스템", "실시간", "공공서비스"]
  },
  {
    id: "welfare-rental-service",
    title: "복지용품·공유물품 대여서비스",
    description: "복지용품과 공유물품을 대여할 수 있는 서비스 플랫폼",
    url: "https://mvp-project-32-gcm09s8qr-dongyeol-jungs-projects.vercel.app/",
    category: "복지",
    date: "2025-10-11",
    tags: ["복지용품", "대여서비스", "공유경제", "사회서비스"]
  },
  {
    id: "jdx-01-ai-education",
    title: "AI 기반 맞춤형 교육 플랫폼",
    description: "바이브에듀 - AI 기반 개인화 학습 경험을 제공하는 교육 플랫폼",
    url: "https://jdx-project-01-mrewf59ag-dongyeol-jungs-projects.vercel.app/",
    category: "교육",
    date: "2025-10-02",
    tags: ["AI", "교육플랫폼", "개인화학습", "바이브에듀"],
    featured: true
  },
  {
    id: "jdx-02-workplace-education",
    title: "직장인을 위한 AI교육 플랫폼",
    description: "직장인들의 업무 역량 향상을 위한 AI 기반 맞춤형 교육 플랫폼",
    url: "https://jdx-project-02-odmxu88t2-dongyeol-jungs-projects.vercel.app/",
    category: "교육",
    date: "2025-10-12",
    tags: ["직장인교육", "AI", "역량개발", "업무교육"]
  },
  {
    id: "music-01-ai-composer",
    title: "AI 작사·작곡 도우미 플랫폼",
    description: "시(詩)를 가사/코드/멜로디로 자동 변환하는 AI 음악 창작 웹앱",
    url: "https://music-project-01-jgqnm0s1m-dongyeol-jungs-projects.vercel.app/",
    category: "AI/음성",
    date: "2025-10-12",
    tags: ["AI", "음악창작", "작사", "작곡", "자동변환"],
    featured: true
  },
  {
    id: "mvp-06-job-platform-renewal",
    title: "AI 중장년 일자리 플랫폼",
    description: "중장년층을 위한 AI 기반 맞춤형 일자리 매칭 플랫폼 리뉴얼",
    url: "https://mvp-project-06-9eengpa8d-dongyeol-jungs-projects.vercel.app/",
    category: "취업지원",
    date: "2025-10-13",
    tags: ["AI", "중장년", "취업매칭", "일자리플랫폼"],
    featured: true
  }
];

// 필터링 함수들
export const getProjectsByCategory = (category: string) => {
  if (category === "all") return projects;
  return projects.filter(project => project.category === category);
};

export const getFeaturedProjects = () => {
  return projects.filter(project => project.featured);
};

export const getProjectById = (id: string) => {
  return projects.find(project => project.id === id);
};

// 카테고리 목록 (실제 프로젝트 카테고리만 포함)
export const categories = [
  "all",
  "웹사이트",
  "플랫폼", 
  "포트폴리오",
  "AI/음성",
  "게임",
  "헬스케어",
  "업무도구",
  "교육",
  "복지",
  "공공서비스",
  "취업지원",
  "의료",
  "관리시스템",
  "숙박/예약"
];

export const categoryLabels: Record<string, string> = {
  "all": "전체",
  "웹사이트": "웹사이트",
  "플랫폼": "플랫폼",
  "포트폴리오": "포트폴리오",
  "AI/음성": "AI/음성",
  "게임": "게임",
  "헬스케어": "헬스케어",
  "업무도구": "업무도구",
  "교육": "교육",
  "복지": "복지",
  "공공서비스": "공공서비스",
  "취업지원": "취업지원",
  "의료": "의료",
  "관리시스템": "관리시스템",
  "숙박/예약": "숙박/예약"
};